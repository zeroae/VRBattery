...
COpenVROverlayController::COpenVROverlayController()
	...
	, m_eOverlayError( vr::VROverlayError_None )
...
bool COpenVROverlayController::Init()
{
	...
	qInfo() << "Offscreen Surface created";
	...
	bSuccess = bSuccess && vr::VROverlay() != NULL;

	if( bSuccess )
	{
		std::string sKey = std::string( "sample." ) + m_strName.toStdString();
		m_eOverlayError = vr::VROverlay()->CreateDashboardOverlay( 
			sKey.c_str(), 
			m_strName.toStdString().c_str(), 
			&m_ulOverlayHandle, 
			&m_ulOverlayThumbnailHandle );
		bSuccess = bSuccess && m_eOverlayError == vr::VROverlayError_None;
	}
...
void COpenVROverlayController::OnSceneChanged( const QList<QRectF>& )
{
	...
	if( !vr::VROverlay() ||
	    ...
	    !vr::VROverlay()->IsOverlayVisible( m_ulOverlayThumbnailHandle ) )
		return;

	qInfo() << "Overlay is visible, rendering.";
	...
	//QImage fboImage(m_pFbo->toImage(!device.paintFlipped()));
	//QImage image(fboImage.constBits(), fboImage.width(), fboImage.height(), QImage::Format_ARGB32);
	//image.save("C:\\Users\\psodr\\hello.png");

	GLuint unTexture = m_pFbo->texture();
	if( unTexture != 0 )
	{
		qInfo() << "Took OpenGL Texture" << unTexture;
		vr::Texture_t texture = {
			(void*)unTexture, 
			vr::TextureType_OpenGL, 
			vr::ColorSpace_Auto 
		};
		m_eOverlayError = vr::VROverlay()->SetOverlayTexture( m_ulOverlayHandle, &texture );
		if (m_eOverlayError != VROverlayError_None) {
			qCritical() << "Error " << m_eOverlayError << " while SetOverlayTexture";
		}
	}
	...
	while( vr::VROverlay()->PollNextOverlayEvent( m_ulOverlayHandle, &vrEvent, sizeof( vrEvent )  ) )
	{
		switch( vrEvent.eventType )
		{
			...
			case vr::VREvent_OverlayShown:
				{
					qInfo() << "OverlayShown: repainting widget";
					m_pWidget->repaint();
				}
				break;
	...
	if( m_ulOverlayThumbnailHandle != vr::k_ulOverlayHandleInvalid )
	{
		while( vr::VROverlay()->PollNextOverlayEvent( m_ulOverlayThumbnailHandle, &vrEvent, sizeof( vrEvent)  ) )
		{
			switch( vrEvent.eventType )
			{
				case vr::VREvent_OverlayShown:
					{
						qInfo() << "Overlay Thumbnail Shown: repainting widget";
						m_pWidget->repaint();
					}
					break;
			}
...
bool COpenVROverlayController::ConnectToVRRuntime()
{
	...
	if ( m_eLastHmdError != vr::VRInitError_None )
	{
        ...
        uint32_t unHmdDeviceIndex = vr::k_unTrackedDeviceIndexInvalid;
	for (uint32_t i=0; i < vr::k_unMaxTrackedDeviceCount; i++)
	{
	  vr::ETrackedDeviceClass deviceClass = pVRSystem->GetTrackedDeviceClass(i);
	  switch(deviceClass) {
	  	case vr::TrackedDeviceClass_Invalid:
			break;
		default:
	  		qInfo() << "Device[" << i << "]: " <<  deviceClass << " " 
				<< GetTrackedDeviceString(pVRSystem, i, 
	...
	m_strVRDriver = GetTrackedDeviceString(pVRSystem, 
	                                       vr::k_unTrackedDeviceIndex_Hmd, 
					       vr::Prop_TrackingSystemName_String);
	m_strVRDisplay = GetTrackedDeviceString(pVRSystem, 
	                                        vr::k_unTrackedDeviceIndex_Hmd, 
						vr::Prop_SerialNumber_String);
